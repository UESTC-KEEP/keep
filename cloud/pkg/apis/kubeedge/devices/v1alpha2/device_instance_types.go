...
// DeviceSpec represents a single devices instance. It is an instantation of a devices model.
type DeviceSpec struct {
	// Required: DeviceModelRef is reference to the devices model used as a template
	// to create the devices instance.
	DeviceModelRef *v1.LocalObjectReference `json:"deviceModelRef,omitempty"`
	// Required: The protocol configuration used to connect to the devices.
	Protocol ProtocolConfig `json:"protocol,omitempty"`
	// List of property visitors which describe how to access the devices properties.
...
type ProtocolConfigCommon struct {
	...
	// Define timeout of mapper collect from devices.
	// +optional
	CollectTimeout int64 `json:"collectTimeout,omitempty"`
	// Define retry times of mapper will collect from devices.
	// +optional
...
type ProtocolConfigBluetooth struct {
	// Unique identifier assigned to the devices.
	// +optional
...
// DeviceStatus reports the devices state and the desired/reported values of twin attributes.
type DeviceStatus struct {
	// A list of devices twins containing desired/reported desired/reported values of twin properties..
	// Optional: A passive devices won't have twin properties and this list could be empty.
	// +optional
	Twins []Twin `json:"twins,omitempty"`
}

// Twin provides a logical representation of control properties (writable properties in the
// devices model). The properties can have a Desired state and a Reported state. The cloud configures
// the `Desired`state of a devices property and this configuration update is pushed to the edge node.
// The mapper sends a command to the devices to change this property value as per the desired state .
// It receives the `Reported` state of the property once the previous operation is complete and sends
// the reported state to the cloud. Offline devices interaction in the edge is possible via twin
// properties for control/command operations.
type Twin struct {
	// Required: The property name for which the desired/reported values are specified.
	// This property should be present in the devices model.
...
// TwinProperty represents the devices property for which an Expected/Actual state can be defined.
...
// DeviceData reports the devices's time-series data to edge MQTT broker.
...
type DeviceData struct {
	...
	// the default value is $ke/events/devices/+/data/update
	// +optional
	DataTopic string `json:"dataTopic,omitempty"`
}

// DataProperty represents the devices property for external use.
...
// DevicePropertyVisitor describes the specifics of accessing a particular devices
// property. Visitors are intended to be consumed by devices mappers which connect to devices
// and collect data / perform actions on the devices.
type DevicePropertyVisitor struct {
	// Required: The devices property name to be accessed. This should refer to one of the
	// devices properties defined in the devices model.
	PropertyName string `json:"propertyName,omitempty"`
	// Define how frequent mapper will report the value.
	// +optional
	ReportCycle int64 `json:"reportCycle,omitempty"`
	// Define how frequent mapper will collect from devices.
	// +optional
	CollectCycle int64 `json:"collectCycle,omitempty"`
	// Customized values for visitor of provided protocols
	// +optional
	CustomizedValues *CustomizedValue `json:"customizedValues,omitempty"`
	// Required: Protocol relevant config details about the how to access the devices property.
...
type VisitorConfigBluetooth struct {
	...
	// Responsible for converting the data coming from the platform into a form that is understood by the bluetooth devices
	// For example: "ON":[1], "OFF":[0]
	//+optional
	DataWriteToBluetooth map[string][]byte `json:"dataWrite,omitempty"`
	// Responsible for converting the data being read from the bluetooth devices into a form that is understandable by the platform
	//+optional
...
// Operations supported by Bluetooth protocol to convert the value being read from the devices into an understandable form
...
// The Modbus register type to read a devices property.
