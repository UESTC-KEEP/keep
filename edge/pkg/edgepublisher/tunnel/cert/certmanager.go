...
func (cm *CertManager) applyCerts() error {
	...
	if err = WriteCert(cm.caFile, ca); err != nil {
	...
	if err = WriteKeyAndCert(cm.keyFile, cm.certFile, pk, crt); err != nil {
...
func (cm *CertManager) rotateCert() (bool, error) {
	...
	if err = WriteKeyAndCert(cm.keyFile, cm.certFile, pk, cert); err != nil {
...
func GetCACert(url string) ([]byte, error) {
	...
	req, err := BuildRequest(nethttp.MethodGet, url, nil, "", "")
	if err != nil {
		return nil, err
	}
...
func (cm *CertManager) GetEdgeCert(url string, capem []byte, cert tls.Certificate, token string) (*ecdsa.PrivateKey, []byte, error) {
	...
	req, err := BuildRequest(nethttp.MethodGet, url, bytes.NewReader(csr), token, cm.NodeName)
	if err != nil {
